...
void drawBlock(int x1, int y1, int x2, int y2, 
  ...
  for (int i = y1; i <= y1 + height ; i++) {
    cmd_addr[*INDEX] = 0x02 + color;
    cmd_addr[*INDEX+1] = x1 << 16 + (y1 + i);
    cmd_addr[*INDEX+2] = x2 << 16 + (y2 + i);
    INDEX[0] = *INDEX + 3;
  }
  cmd_addr[*INDEX] = 0x00000000;
  INDEX[0] = *INDEX + 1;
}
...
void drawPlayer(struct player* playa, int x_dir, 
  ...
  cmd_addr[*INDEX] = 0x01000000;
  cmd_addr[*INDEX+1] = 0x03ff0000;
  cmd_addr[*INDEX+2] = 0x00000030 + (playa->y <<12) + (playa->x << 22);
  cmd_addr[*INDEX+3] = 0x00000000;
  INDEX[0] = *INDEX+4;
}
